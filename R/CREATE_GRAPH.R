...
CreateGraph <- function(profiles,publications){
  perfil <- jsonlite::fromJSON(profiles)
  ...
  public <- jsonlite::fromJSON(publications)
  ...
  edge.list.weight <- edge.list %>% tidyr::unite("V1_V2",c(V1, V2), sep = ";") %>%
    dplyr::group_by_at(1) %>% dplyr::tally() %>% tidyr::separate(1, into = c("V1", "V2"), sep=";")
  ...
  node.list <- node.list %>% dplyr::left_join(id.name, by = "idLattes")

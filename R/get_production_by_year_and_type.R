...
ProductionByYearAndType <- hash::hash()
  ...
  if (type.production =='BIBLIOGRAFICA'){
    ...
    if(default[[type]]=="sim"){
		...
		texts.by.year <- Execute(ids, data, conf.JSON, type.production, 'TEXTO_EM_JORNAIS' ,levenshtein)
      		hash::.set(ProductionByYearAndType, type, texts.by.year)
    ...
    if (type=='incluir_orientacao_concluida_mestrado' && default[[type]]=="sim"){
     ...
     hash::.set(ProductionByYearAndType, type, advised.dissertations.by.year)
     ...
     hash::.set(ProductionByYearAndType, type, postdoctoral.on.going.works.by.year)
      ...
      hash::.set(ProductionByYearAndType, type, postdoctoral.advised.works.by.year)
      ...
      hash::.set(ProductionByYearAndType, type,  on.going.dissertations.by.year)
        ...
        hash::.set(ProductionByYearAndType, type, on.going.monograph.by.year)
        ...
        hash::.set(ProductionByYearAndType, type, on.going.scientific.initiation.by.year)
        ...
        hash::.set(ProductionByYearAndType, type, on.going.theses.by.year)
       ...
       hash::.set(ProductionByYearAndType, type, advised.others.by.year)
     ...
     hash::.set(ProductionByYearAndType, type, advised.theses.by.year )
     }
